...
pub struct LayoutAreas {
    pub title_bar: Rect,
    pub current_slide: Rect,
    pub content: Rect,
}

pub type Effects = EffectManager<EffectKey>;

pub fn blink_effect() -> Effect {
    fx::sequence(&[
...
pub fn slide_transition_effect(area: Rect) -> Effect {
...
pub fn step_reveal_effect(area: Rect) -> Effect {
    fx::fade_from_fg(Color::DarkGray, (300, Interpolation::SineOut)).with_area(area)
}

pub fn dialog_open_effect(area: Rect) -> Effect {
    fx::sweep_in(
        Motion::DownToUp,
        3,
        0,
...
pub fn init_to_running_effect(area: Rect) -> Effect {
    fx::coalesce((700, Interpolation::CubicOut)).with_area(area)
}

pub fn running_to_done_effect() -> Effect {
    fx::parallel(&[
        fx::hsl_shift(
            Some([30.0, 0.2, 0.15]),
            Some([30.0, 0.2, 0.15]),
...
pub fn connection_pulse_effect(area: Rect, connected: bool) -> Effect {
